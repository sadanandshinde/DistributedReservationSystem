...
public class LockManager
{
    ...
    private static int DEADLOCK_TIMEOUT = 100000;
                ...
                synchronized (this.lockTable) {
                    ...
                    if (!bConflict) {
                        ...
                        if (bConvert.get(0) == true) {
                            ...
                            // lock table
							System.out.println("Bitset True");
							...
							System.out.println("upgraded lock in locktable");
            ...
            if (dataObj.getXId() == dataObj2.getXId()) {    
                ...
                if (dataObj.getLockType() == DataObj.READ) {    
                    ...
                    // *** ADD CODE HERE *** to take care of both these cases
						...
						System.out.println("transaction already has a lock and is requesting a WRITE lock");
                	if (dataObj2.getLockType() == DataObj.READ){
                		for (int j = 0; j < size; j++) {
                			DataObj tempObj = (DataObj) vect.elementAt(j);

                			if(tempObj.strData.equals(dataObj.strData)  && tempObj.xid != dataObj.xid){
                				System.out.println("want to upgrade a lock but someone has a shared read lock");
                				return true;
                			}
                		}
						System.out.println("the lock coversion was successfull");
                		// change the lock
                		bitset.set(0, true);
						return false;
						
                		// case 2) - Write lock already present
