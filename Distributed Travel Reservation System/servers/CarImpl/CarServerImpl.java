...
public class CarServerImpl extends ShadowPager implements ResourceManager 
{
    ...
    private static final Boolean ASKFORCRASH = true;
    ...
    public static void main(String args[]) {
        ...
        if (args.length == 1) {
            ...
            System.out.println("Usage: java CarImpl.CarServerImpl [mwServer:host] [mwPort]");
            System.exit(1);
        }
        ...
        try {
            ...
            CarServerImpl obj = new CarServerImpl();
            DBCopyObject dbCopy = obj.initShadowpaging("CarImpl");
                ...
                if(dbCopy.itemsInDBCopy != null) {
                    ...
                    if(!dbCopy.itemsInDBCopy.isEmpty()) {
                        ...
                        for(String key : recoveredHTKeys) {
                            Car car = (Car) dbCopy.itemsInDBCopy.get(key);
                            System.out.println("Key: " + key + ", Value: " + car.toString());
                        }
                    }
                }
            }
            ...
            registry.rebind("PG7CarServer", rm);

            ServerThread serverThread = new ServerThread(obj, RESPONSETIMEOUT);
            serverThread.start();

            System.err.println(" PG7CarServer Server ready");
        } catch (Exception e) {
            System.err.println("PG7CarServer Server exception: " + e.toString());
            e.printStackTrace();
        }
    ...
    public CarServerImpl() throws RemoteException {
    }
    ...
    public ReservableItem getRMEntry(int id, String key) throws RemoteException {
        ...
        return (ReservableItem) readData( id, Car.getKey(key) );
    }
...
//    }
    ...
    */
            ...
            try{
                switch(interruptedTransaction.currentState) {
                    ...
                    case VOTE_YES:
                        ...
                        interruptedTransaction.senderServerName = "car";
                        executor = Executors.newSingleThreadExecutor();
                        ...
                        System.out.println("Coordinator Responded!");
    ...
    public TransactionLogItem transfer2PCMessage(TransactionLogItem receivedLogItem) throws RemoteException {
        System.out.println("Car server received log item with xid = " + receivedLogItem.transactionID + " and state = " + receivedLogItem.currentState);
        ...
        switch(receivedLogItem.currentState) {
            case SEND_VOTEREQ:
                ...
                askToEnforceCrash("Crash Car RM after receive vote request but before sending answer?");
                for(int i = 0; i < response.operationsInTransaction.size(); i++) {
                    ...
                    if(!canBeExecuted) {
                        try{
                            ...
                            executor.submit(() -> {
                                askToEnforceCrash("Crash Car RM after sending answer?");
                            });
                        }
                    }
                }
            ...
            case SEND_COMMIT:
                System.out.println("receivedLogItem.transactionID = " + receivedLogItem.transactionID + ", lastValidXid = " + lastRetreivedValidXid);
                if(receivedLogItem.transactionID > lastRetreivedValidXid) {
                    askToEnforceCrash("Crash Car RM after receiving decision but before committing?");
                    ...
                    if(!receivedLogItem.workingSet.isEmpty()) {
                        ...
                        for(String key : workingSetKeys) {
                            ...
                            Car item = (Car) receivedLogItem.workingSet.get(key);
                            ...
                            if(item != null) {
                                ...
                                System.out.println("Updating car RM hashtable entry with key = " + item.getKey());
                            }
                            else{
                                // effectively deleteRoom
                                removeRMEntry(receivedLogItem.transactionID, item.getKey());
                                System.out.println("REMOVING car RM hashtable entry with key = " + item.getKey());
                            }
                        }
                    }
                    else{
                        System.out.println("Working set is empty!");
                    }
            ...
            case SEND_ABORT:
                if(receivedLogItem.transactionID > lastRetreivedValidXid) {
                    askToEnforceCrash("Crash Car RM after receiving decision but before aborting?");
