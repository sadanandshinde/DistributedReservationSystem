...
public class RoomServerImpl extends ShadowPager implements ResourceManager 
{   
    ...
    public static void main(String args[]) {
        ...
        if (args.length == 1) {
            ...
            System.out.println("Usage: java RoomImpl.RoomServerImpl [mwServer:host] [mwPort]");
            System.exit(1);
        }
        ...
        try {
            ...
            RoomServerImpl obj = new RoomServerImpl();
            DBCopyObject dbCopy = obj.initShadowpaging("RoomImpl");
                ...
                if(dbCopy.itemsInDBCopy != null) {
                    ...
                    if(!dbCopy.itemsInDBCopy.isEmpty()) {
                        ...
                        for(String key : recoveredHTKeys) {
                            Hotel room = (Hotel) dbCopy.itemsInDBCopy.get(key);
                            System.out.println("Key: " + key + ", Value: " + room.toString());
                        }
                    }
                }
            }
            ...
            registry.rebind("PG7RoomServer", rm);

            ServerThread serverThread = new ServerThread(obj, RESPONSETIMEOUT);
            serverThread.start();

            System.err.println(" PG7RoomServer Server ready");
        } catch (Exception e) {
            System.err.println("PG7RoomServer Server exception: " + e.toString());
            e.printStackTrace();
        }
    ...
    public RoomServerImpl() throws RemoteException {
    }
    ...
    public ReservableItem getRMEntry(int id, String key) throws RemoteException {
        ...
        return (ReservableItem) readData( id, Hotel.getKey(key) );
    }
...
//    }
    ...
    */
            ...
            try{
                switch(interruptedTransaction.currentState) {
                    ...
                    case VOTE_YES:
                        ...
                        interruptedTransaction.senderServerName = "room";
                        executor = Executors.newSingleThreadExecutor();
    ...
    public TransactionLogItem transfer2PCMessage(TransactionLogItem receivedLogItem) throws RemoteException {
        System.out.println("Room server received log item with xid = " + receivedLogItem.transactionID + " and state = " + receivedLogItem.currentState);
        ...
        switch(receivedLogItem.currentState) {
            case SEND_VOTEREQ:
                ...
                askToEnforceCrash("Crash Room RM after receive vote request but before sending answer?");
                for(int i = 0; i < response.operationsInTransaction.size(); i++) {
                    ...
                    if(!canBeExecuted) {
                        try{
                            ...
                            executor.submit(() -> {
                                askToEnforceCrash("Crash Room RM after sending answer?");
                            });
                        }
                    }
                }
            ...
            case SEND_COMMIT:
                if(receivedLogItem.transactionID > lastRetreivedValidXid) {
                    askToEnforceCrash("Crash Room RM after receiving decision but before committing?");
                    ...
                    if(!receivedLogItem.workingSet.isEmpty()) {
                        ...
                        for(String key : workingSetKeys) {
                            Hotel item = (Hotel) receivedLogItem.workingSet.get(key);
                            if(item != null) {
                                ...
                                System.out.println("Updating room RM hashtable entry with key = " + item.getKey());
                            }
                            else{
                                // effectively deleteRoom
                                removeRMEntry(receivedLogItem.transactionID, item.getKey());
                                System.out.println("REMOVING room RM hashtable entry with key = " + item.getKey());
                            }
                        }
                    }
            ...
            case SEND_ABORT:
                if(receivedLogItem.transactionID > lastRetreivedValidXid) {
                    askToEnforceCrash("Crash Room RM after receiving decision but before aborting?");
